...
class HdfsParquetTableWriter : public HdfsTableWriter {
 public:
  ...
  /// Make an async call to append rows in batch.
  virtual Status AppendRowBatch(boost::shared_ptr<RowBatch>& batch,
  ...
  virtual void Shutdown(bool eos);
 ...
 private:
  ...
  /// Estimate rows per file to limit the file size within Hdfs block size.
  int64_t estimate_rows_per_file_;
  /// Create one thread per ColumnWriter.
  ThreadGroup writer_thread_group_;

  boost::mutex lock_; // protects all following fields
  volatile bool shut_down_;
  volatile bool eos_;
  /// Number of active writer threads. The last active writer thread is responsible for
  /// finalizing the current file.
  int num_writer_thread_;
  /// Condition variable to signal the writer threads that the finalize work is done.
  boost::condition_variable finalize_partition_file_cv_;
};
