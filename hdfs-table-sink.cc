...
Status HdfsTableSink::Send(RuntimeState* state, RowBatch* batch, bool eos) {
  if (default_partition_->file_format() == THdfsFileFormat::PARQUET) {
    return SendAsync(state, batch, eos);
  }
...
Status HdfsTableSink::SendAsync(RuntimeState* state, RowBatch* batch, bool eos) {
  SCOPED_TIMER(runtime_profile_->total_time_counter());
  ExprContext::FreeLocalAllocations(output_expr_ctxs_);
  ExprContext::FreeLocalAllocations(partition_key_expr_ctxs_);
  RETURN_IF_ERROR(state->CheckQueryState());
  ...
  if (dynamic_partition_key_expr_ctxs_.empty()) {
    ...
    if (!has_empty_input_batch_) {
      ...
      row_batch_.reset(new RowBatch(row_desc_, state->batch_size(), mem_tracker_.get()));
      row_batch_->AcquireState(batch);
      row_group_indices_.reset();
      RETURN_IF_ERROR(output_partition->writer->AppendRowBatch(
          row_batch_, row_group_indices_));
    }
  } else {
    for (int i = 0; i < batch->num_rows(); ++i) {
      current_row_ = batch->GetRow(i);
    ...
    for (PartitionMap::iterator partition = partition_keys_to_output_partitions_.begin();
      ...
      row_batch_.reset(new RowBatch(row_desc_, state->batch_size(), mem_tracker_.get()));
      row_batch_->AcquireState(batch);
      row_group_indices_.reset(new vector<int32_t>(partition->second.second.begin(),
          partition->second.second.end()));
      RETURN_IF_ERROR(output_partition->writer->AppendRowBatch(
          row_batch_, row_group_indices_));
      partition->second.second.clear();
    }
  ...
  if (eos) {
    ...
    for (PartitionMap::iterator cur_partition =
        ...
        cur_partition != partition_keys_to_output_partitions_.end();
        ++cur_partition) {
      OutputPartition* output_partition = cur_partition->second.first;
      output_partition->writer->Shutdown(eos);
    }
    return status_;
  }
