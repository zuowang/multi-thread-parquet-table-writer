...
typedef boost::shared_ptr<vector<int32_t> > RowGroupIndices;
...
class HdfsTableSink : public DataSink {
 public:
  ...
  /// Make an async call to append all rows in batch.
  virtual Status SendAsync(RuntimeState* state, RowBatch* batch, bool eos);
  ...
  const RowDescriptor& row_desc() { return row_desc_; }
