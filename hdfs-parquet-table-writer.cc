...
class HdfsParquetTableWriter::PartitionRowBatch : public InternalQueue<PartitionRowBatch>::Node {
 public:
   PartitionRowBatch(boost::shared_ptr<RowBatch>& row_batch,
       RowGroupIndices& row_group_indices)
  : row_batch_(row_batch),
    row_group_indices_(row_group_indices) {
   }

 private:
   friend class HdfsParquetTableWriter::BaseColumnWriter;
   boost::shared_ptr<RowBatch> row_batch_;
   RowGroupIndices row_group_indices_;
};
...
class HdfsParquetTableWriter::BaseColumnWriter {
 public:
  ...
  BaseColumnWriter(HdfsParquetTableWriter* parent, ExprContext* expr_ctx,
    ...
    : parent_(parent), expr_ctx_(expr_ctx), codec_(codec),
      ...
      reusable_col_mem_pool_(new MemPool(parent_->parent_->mem_tracker())),
      per_file_mem_pool_(new MemPool(parent_->parent_->mem_tracker())) {
    Codec::CreateCompressor(NULL, false, codec, &compressor_);

    def_levels_ = parent_->state_->obj_pool()->Add(
        new RleEncoder(reusable_col_mem_pool_->Allocate(DEFAULT_DATA_PAGE_SIZE),
                       DEFAULT_DATA_PAGE_SIZE, 1));
    values_buffer_ = reusable_col_mem_pool_->Allocate(values_buffer_len_);
  }
  ...
  void Close() {
    ...
    PartitionRowBatch* part_row_batch = NULL;
    while (part_row_batch = part_row_batches_.Dequeue()) {
      delete part_row_batch;
    }
  }
  ...
  AtomicInt<int> num_remaining_rows_;
 ...
 protected:
  ...
  boost::condition_variable work_available_; // protected by parent_->lock_

  /// List of batches to be append.
  InternalQueue<PartitionRowBatch> part_row_batches_;
...
void HdfsParquetTableWriter::BaseColumnWriter::WorkLoop() {
  while (true) {
    {
      unique_lock<mutex> writer_lock(parent_->lock_);
      while (!parent_->shut_down_ && !parent_->eos_ && part_row_batches_.empty()) {
        work_available_.wait(writer_lock);
      }
      if (parent_->shut_down_) break;
      if (parent_->eos_ && part_row_batches_.empty()) {
        --parent_->num_writer_thread_;
        if (parent_->num_writer_thread_ == 0) {
          parent_->row_count_ = parent_->estimate_rows_per_file_ - num_remaining_rows_;
          parent_->output_->num_rows += parent_->row_count_;
          VLOG_QUERY << "finalize " << parent_->row_count_ << " rows."
          << parent_->estimate_rows_per_file_ << endl;
          lock_guard<mutex> l(parent_->parent_->lock_);
          parent_->parent_->FinalizePartitionFile(parent_->state_, parent_->output_);
        }
        break;
      }
      DCHECK(!part_row_batches_.empty());
    }

    boost::scoped_ptr<PartitionRowBatch> batch(part_row_batches_.Dequeue());
    DCHECK(batch.get());
    int limit = 0;
    if (batch->row_group_indices_.get() != NULL) {
      limit = batch->row_group_indices_->size();
    } else {
      limit = batch->row_batch_->num_rows();
    }
    Status s;
    for (int row_idx = 0; row_idx < limit; row_idx++) {
      TupleRow* current_row = batch->row_group_indices_ ?
          batch->row_batch_->GetRow((*batch->row_group_indices_)[row_idx]) :
          batch->row_batch_->GetRow(row_idx);
      s = AppendRow(current_row);
      if (s.ok()) {
        --num_remaining_rows_;
        if (num_remaining_rows_ == 0) {
          unique_lock<mutex> l(parent_->lock_);
          --parent_->num_writer_thread_;
          if (parent_->num_writer_thread_ == 0) {
            parent_->row_count_ = parent_->estimate_rows_per_file_;
            parent_->output_->num_rows += parent_->row_count_;
            VLOG_QUERY << "finalize " << parent_->row_count_ << "rows." << endl;
            lock_guard<mutex> l(parent_->parent_->lock_);
            parent_->parent_->FinalizePartitionFile(parent_->state_, parent_->output_);
            parent_->parent_->CreateNewTmpFile(parent_->state_, parent_->output_);
            per_file_mem_pool_->Clear();
            parent_->num_writer_thread_ = parent_->columns_.size();
            parent_->finalize_partition_file_cv_.notify_all();
          } else {
            parent_->finalize_partition_file_cv_.wait(l);
          }
          //num_remaining_rows_ = parent_->estimate_rows_per_file_;
        }
      } else {
        boost::lock_guard<boost::mutex> l(parent_->parent_->lock_);
        parent_->parent_->status_ = s;
        break;
      }
    }
  }
  DCHECK(parent_->shut_down_ || parent_->eos_);
}
...
class HdfsParquetTableWriter::ColumnWriter :
 ...
 public:
  ...
  virtual void Reset() {
    ...
    dict_encoder_.reset(
        new DictEncoder<T>(per_file_mem_pool_.get(), encoded_value_size_));
  ...
  *first_dictionary_page = -1;
  ...
  if (dict_encoder_base_ != NULL) {
    *first_dictionary_page = *file_pos;
    ...
    uint8_t* dict_buffer = per_file_mem_pool_->Allocate(
    ...
    if (compressor_.get() != NULL) {
      ...
      uint8_t* compressed_data =
          per_file_mem_pool_->Allocate(max_compressed_size);
      ...
      per_file_mem_pool_->ReturnPartialAllocation(
...
void HdfsParquetTableWriter::BaseColumnWriter::FinalizeCurrentPage() {
  ...
  if (compressor_.get() == NULL) {
    uncompressed_data =
        per_file_mem_pool_->Allocate(header.uncompressed_page_size);
  } else {
    // We have compression.  Combine into the staging buffer.
    compression_staging_buffer_.resize(
        header.uncompressed_page_size);
    uncompressed_data = &compression_staging_buffer_[0];
  }
  ...
  if (compressor_.get() == NULL) {
    ...
    int64_t max_compressed_size =
        compressor_->MaxOutputLen(header.uncompressed_page_size);
    DCHECK_GT(max_compressed_size, 0);
    uint8_t* compressed_data = per_file_mem_pool_->Allocate(max_compressed_size);
    header.compressed_page_size = max_compressed_size;
...
Status HdfsParquetTableWriter::Init() {
  ...
  for (int i = 0; i < columns_.size(); ++i) {
    stringstream ss;
    ss << "work-loop: " << i;
    writer_thread_group_.AddThread(new Thread("parquet-column-writer", ss.str(),
        &BaseColumnWriter::WorkLoop, columns_[i]));
  }
  num_writer_thread_ = columns_.size();
  return Status::OK();
}
...
Status HdfsParquetTableWriter::InitNewFile() {
  ...
  int64_t file_block_size;
  RETURN_IF_ERROR(HdfsTableSink::GetFileBlockSize(output_, &file_block_size));
  estimate_rows_per_file_ = file_block_size / parent_->row_desc().GetRowSize();
  for (int i = 0; i < columns_.size(); ++i) {
    columns_[i]->num_remaining_rows_ = estimate_rows_per_file_;
  }
...
Status HdfsParquetTableWriter::AppendRowBatch(boost::shared_ptr<RowBatch>& batch,
    RowGroupIndices& row_group_indices) {
  boost::lock_guard<boost::mutex> l(lock_);
  for (int i = 0; i < columns_.size(); ++i) {
    columns_[i]->part_row_batches_.Enqueue(new PartitionRowBatch(batch, row_group_indices));
    columns_[i]->work_available_.notify_one();
  }
  return Status::OK();
}
...
void HdfsParquetTableWriter::Shutdown(bool eos) {
  DCHECK(!eos_ && !shut_down_);
  if (eos) {
    eos_ = true;
  } else {
    shut_down_ = true;
...

  {
    ...
    unique_lock<mutex> writer_lock(lock_);
  }
  // Notify all writer threads we reach the eos.
  for (int i = 0; i < columns_.size(); ++i) {
    columns_[i]->work_available_.notify_one();
  }
  writer_thread_group_.JoinAll();
}
